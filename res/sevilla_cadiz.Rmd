...
## Estudio de los datos sin logartimos
...
train_log <- window(l.ts_mensual, start=c(1990, 01), end=c(2014, 12)) 
plot(train_log)
```

```{r}

test_log <- window(l.ts_mensual, start=c(2015, 01), end=c(2016, 12))
plot(test_log)
```

###3-B-Modelado exponencial ETS y predicción
```{r}
fit_log <- ets(train_log)
fit_log
```

```{r}
prediccion3 <- forecast(fit_log)
```
...
Precisión:
```{r}
accuracy(prediccion3,test_log)
```

###4-B-Modelado ARIMA y predicción:
```{r}
fit2_log <- auto.arima(train_log)
fit2_log
```


```{r}
prediccion4 <- forecast(fit2_log)
prediccion4
```
...
Analizando los residuos: No se ve correlación, la distribución de residuos es normal y pasa el test de de portmanteau, luego los intervalos de predicción son fiables 
```{r}
tsdisplay(residuals(fit2_log))

```

```{r}
hist(residuals(fit2_log))

```

```{r}
Box.test(residuals(fit2_log),  type="Ljung")
```

## Resumen de las predicciones anteriores


Creo que de estas tablas sacaría la información para Shiny:
```{r}
#prediccion con ETS si aplicar log
tabla_shiny1 <- as.data.frame(prediccion1)
#prediccion con ARIMA si aplicar log
tabla_shiny2 <- as.data.frame(prediccion2)
#prediccion con ETS aplicando log
tabla_shiny3 <- as.data.frame(prediccion3)
#prediccion con ARIMA aplicando log
tabla_shiny4 <- as.data.frame(prediccion4)
```

Construyo una tabla comparativa con todos los modelos
```{r}
a <- as.data.frame(accuracy(prediccion1, test))
a$modelo <- "ETS"
b <- as.data.frame(accuracy(prediccion2, test))
b$modelo <- "ARIMA"
c <- as.data.frame(accuracy(prediccion3, test_log))
c$modelo  <- "ETS_log"
d <- as.data.frame(accuracy(prediccion4, test_log))
d$modelo <- "ARIMA_log"
compara_predicciones_modelos <- rbind(a, b, c, d)
kable(compara_predicciones_modelos)
```
